 ...
 *
 * Generates a new AI interview brief for a calendar event.
 * Validates that the event is marked as an interview and belongs to the authenticated user.
 */
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // 5. Set brief status to 'in_progress' (idempotent upsert)
    ...
    if (upsertError) {
      console.error('[Generate Brief] Upsert error:', upsertError);
      return NextResponse.json(
        { error: 'Failed to initialize brief generation' },
        { status: 500 }
      );
    }

    // 6. Build prompt from event data
    ...
    // 7. Call Gemini (with retry on parse failure)
    let briefData;
    ...
    try {
      ...
      if (!validation.success) {
        ...
        console.warn('[Generate Brief] Initial validation failed, retrying...');
      ...
      console.error('[Generate Brief] LLM error:', llmError);
    ...
    if (saveError) {
      console.error('[Generate Brief] Save error:', saveError);
    ...
    console.error('[Generate Brief] Unexpected error:', error);
