...
import type { WindowPosition, TranscriptUpdateData } from "../../types/unified-recording";
...
export class FloatingWindowManager {
  ...
  async create(): Promise<void> {
    ...
    if (isDev && process.env["ELECTRON_RENDERER_URL"]) {
      ...
      await this.window.loadURL(`${process.env["ELECTRON_RENDERER_URL"]}/floating.html`);
    } else {
      // Production: load built file
      const htmlPath = join(__dirname, "../renderer/floating.html");
      await this.window.loadFile(htmlPath);
    }

    console.log(`[FloatingWindowManager] Loading floating window HTML, isDev: ${isDev}`);
    ...
    console.log("[FloatingWindowManager] Window created");
  }
  ...
  togglePosition(): void {
    ...
    console.log(`[FloatingWindowManager] Position toggled to: ${this.position}`);
  }
